(function() {
	...
	function Collection(kind, options) {
		...
		this.get = function(id, params, getCallback) {
			if (!id) {
				throw new Error('Cannot get ' + inflection.singularize(kind) + ' - ID must be given.');
			}

			// params is optional
			if (_.isFunction(params) && _.isUndefined(getCallback)) {
				getCallback = params;
				params = {};
			}

			// We don't need to do a full get request, since we just want to call on a subitem
			if (params.subitem) {
				return getCallback(null, new CollectionItem(kind, {}, id, options), {}, {}, {});
			}
